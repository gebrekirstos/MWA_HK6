...
app.post('/contactus', urlencodedParser, function (req, res) {  //what I added
  ...
  if(!req.body.comment) errors.push({msg:'Give us your comment, Please?'});
  if(errors) {
  ...
  if(errors.length==0){
      ...
      function comment(fname, cmmtOpt, commentBody, ip){
        this.fname = fname;
        this.cmmtOpt = cmmtOpt;
...
app.get('/contactus', csrfProtection, function(req, res) {      //what I added
  ...
  console.log('my talken is ===>: '+req.csrfToken());
...
function writetoFile(comObj){
  var text="";
  text= 'First name: '+comObj.fname;
  text+= '\r\t Comment Type: '+comObj.cmmtOpt;
  text+= '\r\t Comment: '+comObj.commentBody;
  text+= '\r\t User\'s IP Address: '+comObj.ipaddress;
  text+='\r\n';
  
  fs.appendFile(__dirname +'/outputFile.txt', text, encoding='utf8', function (err) {
    if (err) throw err;
  });
}
module.exports = app;
